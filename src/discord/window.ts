...
async function queueIDMDownload(passedWindow: BrowserWindow, url: string, filename?: string): Promise<boolean> {
    const headers = await buildGopeedRequestHeaders(passedWindow, url);
    const taskOptions: DownloadManagerTaskOptions = filename
        ? {
              filename,
              headers,
          }
    ...
    try {
        ...
        console.error("Failed to queue download to IDM:", error);
        return false;
    }
}
...
async function queueDownloadToManager(
    ...
    if (manager === "gopeed") {
        return queueGopeedDownload(passedWindow, url, filename);
    } else if (manager === "idm") {
        return queueIDMDownload(passedWindow, url, filename);
    }
    return false;
}
